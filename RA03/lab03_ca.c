...
float* threaded_mse_ma(int **x, int q, int n, int t) {
    ...
    for (int i = 0; i < t; i++) {
        // Optimized from LRP02, no longer puts all remaining in the last thread, but distributes
        ...
        // Create thread suspended to set core affinity before it starts running
        tids[i] = CreateThread(NULL, 0, mse_ma, &arguments[i], CREATE_SUSPENDED, NULL);
        if (tids[i] != NULL) {
            
            // Basis: 8 Logical Processor PC, used 7 LPs
            // Assign thread from LP 0-6
            DWORD_PTR affinityMask = (DWORD_PTR)(1ULL << (i % 7));
            SetThreadAffinityMask(tids[i], affinityMask);
            ResumeThread(tids[i]);
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    if (argc == 2) {
        ...
        // Allocate and populate the random matrix
        matrix = (int**)malloc(n * sizeof(int*));
        for (int i = 0; i < n; i++) {
            matrix[i] = (int*)malloc(n * sizeof(int));
            for (int j = 0; j < n; j++) {
    ...
    // Use Windows QueryPerformanceCounter for high-resolution timing (help from AI)
